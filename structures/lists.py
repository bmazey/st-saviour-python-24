def evens_only(numbers: []) -> []:
    ...
    while i < len(numbers):
        ...
        if numbers[i] % 2 == 0:
            evens.append(numbers[i])  # to add even number to the result list
        i += 1
    
    return evens

# extracts the last digit from four-digit integers in a list
def last_of_four_digits(numbers: []) -> []:
    ...
    while i < len(numbers):
        bag = numbers[i] % 10  # Extracts the last digit
...
# rounds up decimal numbers in a list to the nearest integer
def round_up(numbers: []) -> []:
    ...
    # Iterates through the numbers list
    while i < len(numbers):
        bobao = numbers[i] % 1  # Gets the decimal part of the number
        if bobao >= 0.5:
            bobao = int(numbers[i]) + 1  # Rounds up if decimal >= 0.5
        else:
            bobao = int(numbers[i])  # (Rounds down if decimal < 0.5)
        boba.append(bobao)  # Adds the rounded number to the result list
        i += 1
    
    return boba

# finds the position of the first negative number in a list
def find_negative(numbers: []) -> int:
    ...
    # Iteratess through the numbers list
    while i < len(numbers):
        if numbers[i] < 0:
